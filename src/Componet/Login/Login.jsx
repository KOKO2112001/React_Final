   ...
   <div className='container my-5 py-5'>
    ...
    {errMsg !="" ?<div className='alert alert-danger'>{errMsg}</div> : ""}
      <form onClick={form1.handleSubmit} >
       ...
       <div>
        {Loding ?<button disabled={!(form1.isValid && form1.dirty)} type='submit' className='btn bg-main text-white'>Login</button> :  <button type='button' className='btn bg-main text-white'> <i className='fa-solid fa-spinner fa-spin'></i></button>}
       </div>
       <br />
      </form>
      <Link to='/forgetbassord' className='cursor-pointer'>Forget Passowrd....?</Link>
   </div>
   </>
  )
}
