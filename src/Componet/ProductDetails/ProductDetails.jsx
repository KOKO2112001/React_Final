...
export default function ProductDetails() {
  ...
  let {data , isLoading} = useQuery(["productDetails",prodectId],getPrudctDetails)
    ...
    {isLoading ? <div className=' loading d-flex justify-content-center align-items-center bg-black position-fixed top-0 end-0 start-0 bottom-0'>
      ...
      <div className='row align-items-center'>
          ...
          <div className='row g-0 align-items-center'>
            <div className=' col-md-2'>
              {data?.data.data.images.map((element)=>{
                return <img src={element} onClick={setImges} className=' w-100 imgs' alt="" />
              })}
            </div>
            <div className=' col-md-10'>
            <img src={data?.data.data.imageCover} id='myImage' className=' w-100' alt="" />
            </div>
          </div>
        </div>
        <div className=' col-md-9'>
          ...
          <button onClick={()=>addToCart(data?.data.data.id)} className='btn bg-main text-white d-block cursor-pointer my-3 w-75'>Add Product</button>
        </div>
      </div>
    </div>}
    </>
