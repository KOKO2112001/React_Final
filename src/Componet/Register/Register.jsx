...
export default function Register() {
  ...
  let validation= Yup.object({
    name:Yup.string().min(3,"min  char 3").max(20,"max char 20").required("Name Required"),
    ...
    rePassword:Yup.string().oneOf([Yup.ref("password")],"confirm Password Not Match").required("Password Required"),
  })
  let form1= useFormik({
    initialValues:{
      name:"",
   ...
   <div className='container my-5 py-5'>
    ...
    <form onClick={form1.handleSubmit} >
      ...
      <div className='mb-3'>
       ...
       <div>
        {Loding ?<button disabled={!(form1.isValid && form1.dirty)} type='submit' className='btn bg-main text-white'>Rigester</button> :  <button type='button' className='btn bg-main text-white'> <i className='fa-solid fa-spinner fa-spin'></i></button>}
        </div>
       </form>
   </div>
   </>
